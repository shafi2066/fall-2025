...
function nested_logit_with_Z(theta, X, Z, y, nesting_structure)
    ...
    # First K columns for WC nest, next K for BC nest, zeros for normalized (last) alternative
    bigAlpha = zeros(K, J)
    ...
    for j = 1:J
        ...
        elseif j in nesting_structure[2]  # Blue collar
            ...
            lidx[:,j] = exp.(X*bigAlpha[:,j] .+ (Z[:,j] .- Z[:,J])*gamma)
        end
    end

    # Compute inclusive value for each nest
    for j = 1:J
