...
function load_dynamic_data()
    ...
    Xstate = Matrix(df[:, r"^Xst\d+"])   # Discretized mileage state (N × T)  <-- FIXED
...
function estimate_static_model(df_long)
    ...
    return theta_static  # Return the model instead of nothing
end
        ...
        for b in 0:1
            for z in 1:d.zbin
                for x in 1:d.xbin
                    ...
                    # store expected value (log-sum-exp form)
